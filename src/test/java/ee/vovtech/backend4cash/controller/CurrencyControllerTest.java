...
class CurrencyControllerTest extends RestTemplateTests {
    ...
    void buyCoins() {
        ResponseEntity<Boolean> exchange = testRestTemplate.exchange("/coins/" + "bitcoin" + "/buy" + "?amount=1&userId=3",
        ...
        ResponseEntity<LoggedInUserDto> userExchange = testRestTemplate.exchange("/users/3",
        ...
        ResponseEntity<Boolean> exchangeFalse = testRestTemplate.exchange("/coins/" + "bitcoin" + "/buy" + "?amount=100000000&userId=3",
                HttpMethod.PUT, new HttpEntity<>(authorizationHeader(userToken)), Boolean.class);
    ...
    void sellCoins() {
        ResponseEntity<Boolean> exchange = testRestTemplate.exchange("/coins/" + "bitcoin" + "/sell" + "?amount=1&userId=3",
                HttpMethod.PUT, new HttpEntity<>(authorizationHeader(userToken)), Boolean.class);
        Boolean response = assertOk(exchange);
        assertTrue(response);
        // get user info to see his coins and cash
        ...
        ResponseEntity<Boolean> exchangeFalse = testRestTemplate.exchange("/coins/" + "bitcoin" + "/sell" + "?amount=1&userId=3",
                HttpMethod.PUT, new HttpEntity<>(authorizationHeader(userToken)), Boolean.class);
        Boolean responseFalse = assertOk(exchangeFalse);
        assertFalse(responseFalse);
    }
